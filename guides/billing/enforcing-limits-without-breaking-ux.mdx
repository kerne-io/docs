...
Don't wait for a "Save" click to inform users they've hit a limit - proactively disable buttons or show upgrade nudges, using [`useEntitlement`](/sdks/client/react/billing/check-entitlement) (which already handles the loading state):

```tsx
import { useEntitlement } from '@kerne/react';

function AddTeamMember() {
  const { hasAccess, isLoading } = useEntitlement('team_seats');

  return (
    <button disabled={isLoading || !hasAccess}>
      Add member {!isLoading && !hasAccess && ' (Upgrade required)'}
    </button>
  );
}
```
