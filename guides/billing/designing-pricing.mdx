...
**What goes in each tier?** This is what makes "Free" different from "Pro", and it's decided once per tier.

**What happens when someone wants more than their tier includes?** Do they hit a wall and have to upgrade, or do they keep going and pay for the excess?

The second one is decided per *thing you're limiting*, not per tier — and the answer can be different for every one of them. A single tier can hold something that hard-stops at five, something that keeps serving and charges for the excess, and something that's simply free forever.

<Note>
  The vocabulary, once: a **feature** is one thing you might limit or grant. What a tier gives someone
  for that feature is an **entitlement**. A **plan** is the tier itself, and a **price** is what it
  costs. Your code checks the entitlement, never the plan name:
</Note>
...
The first covers most add-ons and needs nothing new — it is [Option B](#option-b-keep-going-and-charge-for-it-pay-as-you-go-past-the-included-amount) with an allowance of `0` and the ceiling left empty. Combine it with [Option C](#option-c-the-limit-isnt-one-pool) when the add-on is bought per project or per seat rather than once per account.
