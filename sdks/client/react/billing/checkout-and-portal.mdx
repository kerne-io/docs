...
function UpgradeButton({ planPriceId }: { planPriceId: string }) {
  ...
  return (
    <button onClick={() => openCheckout(planPriceId)}>
      Upgrade
    </button>
  );
}
