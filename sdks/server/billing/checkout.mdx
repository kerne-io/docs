...
<Note>
  Auth callbacks (verify email, reset password, magic link) resolve the same way - unconditionally
  to the portal, which then hands off to `portal_redirect_url` or `app_url`.
</Note>
...
1. **`createSetupIntent()`** - a Stripe SetupIntent for your own Stripe.js / Payment Element form. No redirect, no charge.
   ...
   ```typescript
   const { clientSecret, customerId } = await kerne.billing.createSetupIntent({ customer: userId });
   ```
...
<Note>
  For CI or a seed script with no browser to confirm a SetupIntent, pass `testCardToken` (e.g.
  `"tok_visa"`) instead of `stripePaymentMethodId` - it mints a fresh payment method server-side on
  each call. Stripe **test mode only**; rejected otherwise.
</Note>
